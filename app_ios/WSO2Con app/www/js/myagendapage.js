...
$('#myagenda').live('pageshow', function (event) {
    if (testmode) {
        console.log("pageshow #myagenda");
    }
    
    if(user != null){
        $("#myagenda-settings").show();
	}else{
		$("#myagenda-settings").hide();
	}
    var currentSession = 0;
    $('#myagendatab2').addClass('ui-btn-active');
    var currentDay;
    if (testdateoffset > 0) {
    ...
    for (var s in agenda.sessions) {
        ...
        if(jQuery.inArray(session.id, myagenda) != -1){
        ...
        if (session.type != "break") {
            template += "</a><a  href='#removemyagenda' class='linkButton' data-icon='minus' data-id='"+session.id+"'></a>";
        }
        template += "</li>";
    ...
    if(count == 0){
    	template = "<center>You have no events in your agenda</center>";
    }

    $("#myagendadetails").html(template).trigger("create");
    ...
    $('.linkButton').click(function(){
    	...
    	myagenda.splice(myagenda.indexOf(currentSessionID),1);
    	window.localStorage.setItem("myagenda", JSON.stringify(myagenda));
    	
    	$.mobile.changePage($("#myagenda"),
    			 {
